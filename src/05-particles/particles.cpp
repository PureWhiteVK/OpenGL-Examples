...
static void draw_particles(GLFWwindow *window, double t, float dt) {
  ...
  while (!glfwWindowShouldClose(window) &&
    ...
    thread_sync.p_done.wait_for(lock, ch::milliseconds(100));
  }
...
static void draw_scene(GLFWwindow *window, double t) {
  ...
  projection =
      glm::perspective(65.f * (float)M_PI / 180.f, aspect_ratio, 1.0f, 60.0f);
...
static int physics_thread_main(void *arg) {
  ...
  for (;;) {
    ...
    while (!glfwWindowShouldClose(window) &&
      ...
      namespace ch = std::chrono;
      thread_sync.d_done.wait_for(lock, ch::milliseconds(100));
    }
...
int main(int argc, char **argv) {
  ...
  physics_thread = std::thread{physics_thread_main, window};
