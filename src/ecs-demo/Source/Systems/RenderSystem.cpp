...
#define SHADER_PATH(relative_path) SOURCE_DIR "/" relative_path

void RenderSystem::Init() {
  gCoordinator.AddEventListener(METHOD_LISTENER(
      Events::Window::RESIZED, RenderSystem::WindowSizeListener));

  shader = std::make_unique<Shader>(SHADER_PATH("Shaders/vertex.glsl"),
                                    SHADER_PATH("Shaders/fragment.glsl"));
  ...
  gCoordinator.AddComponent(
      mCamera, Camera{.projectionTransform = Camera::MakeProjectionTransform(
                          45.0f, 0.1f, 1000.0f, 1920, 1080)});

  std::vector<Vec3> vertices = {
      Vec3(-0.5f, -0.5f, -0.5f), Vec3(0.5f, -0.5f, -0.5f),
      Vec3(0.5f, 0.5f, -0.5f),   Vec3(0.5f, 0.5f, -0.5f),
      Vec3(-0.5f, 0.5f, -0.5f),  Vec3(-0.5f, -0.5f, -0.5f),
      Vec3(-0.5f, -0.5f, 0.5),   Vec3(0.5f, -0.5f, 0.5),
      Vec3(0.5f, 0.5f, 0.5),     Vec3(0.5f, 0.5f, 0.5),
      Vec3(-0.5f, 0.5f, 0.5),    Vec3(-0.5f, -0.5f, 0.5),
      Vec3(-0.5f, 0.5f, 0.5f),   Vec3(-0.5f, 0.5f, -0.5f),
      Vec3(-0.5f, -0.5f, -0.5f), Vec3(-0.5f, -0.5f, -0.5f),
  ...
  std::vector<Vec3> normals = {
      ...
      Vec3(0.0f, -1.0f, 0.0f), Vec3(0.0f, -1.0f, 0.0f), Vec3(0.0f, -1.0f, 0.0f),
      Vec3(0.0f, 1.0f, 0.0f),  Vec3(0.0f, 1.0f, 0.0f),  Vec3(0.0f, 1.0f, 0.0f),
      Vec3(0.0f, 1.0f, 0.0f),  Vec3(0.0f, 1.0f, 0.0f),  Vec3(0.0f, 1.0f, 0.0)};
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3) * normals.size(), normals.data(),
               GL_STATIC_DRAW);
...
void RenderSystem::WindowSizeListener(Event &event) {
  auto windowWidth =
      event.GetParam<unsigned int>(Events::Window::Resized::WIDTH);
  auto windowHeight =
      event.GetParam<unsigned int>(Events::Window::Resized::HEIGHT);

  auto &camera = gCoordinator.GetComponent<Camera>(mCamera);
  camera.projectionTransform = Camera::MakeProjectionTransform(
      45.0f, 0.1f, 1000.0f, windowWidth, windowHeight);
}
